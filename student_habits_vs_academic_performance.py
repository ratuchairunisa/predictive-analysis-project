# -*- coding: utf-8 -*-
"""student_habits_vs_academic_performance.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1AYP-eR0L6otT9-CBFa5MK0N5GX96KTbk
...
"""Insight: dari informasi yang ditampilkan terdapat missing value pada fitur parental_education_level
...
### Mengkategorikan Fitur Numerik & Kategorik
"""
...
# Melihat distribusi pada kolom numerik menggunakan histogram
num_col = ['study_hours_per_day', 'social_media_hours', 'netflix_hours', 'attendance_percentage', 'sleep_hours', 'exam_score']
df[num_col].hist(bins=20, figsize=(10, 8))
plt.tight_layout()
plt.show()

"""Insight:

1. **study_hours_per_day (Jam Belajar per Hari):**
* Distribusi: Distribusi ini tampak condong ke kanan (right-skewed).
* Penjelasan: Sebagian besar siswa belajar antara 1 hingga 4 jam per hari, dengan puncak (modus) berada di sekitar 2 hingga 3 jam. Jumlah siswa yang belajar lebih lama (lebih dari 4 jam) semakin sedikit, menunjukkan bahwa hanya sebagian kecil siswa yang menghabiskan waktu sangat lama untuk belajar setiap hari.

2. **social_media_hours (Jam Media Sosial):**
* Distribusi: Distribusi ini juga terlihat agak condong ke kanan (slightly right-skewed).
* Penjelasan: Puncak penggunaan media sosial berada di sekitar 2.5 hingga 3.5 jam per hari. Ada cukup banyak siswa yang menggunakan media sosial antara 1 hingga 4 jam. Jumlah siswa yang menggunakan media sosial lebih dari 4 jam cenderung menurun.

3. **netflix_hours (Jam Menonton Netflix):**
* Distribusi: Distribusi ini tampak condong ke kanan (right-skewed) dan mungkin bimodal atau memiliki puncak ganda.
* Penjelasan: Ada puncak yang signifikan di sekitar 1 hingga 2 jam, menunjukkan banyak siswa menonton Netflix dalam rentang waktu tersebut. Ada juga kelompok siswa yang menonton sangat sedikit (mendekati 0 jam). Jumlah siswa yang menonton lebih dari 2 jam semakin berkurang.

**4. attendance_percentage (Persentase Kehadiran):**
* Distribusi: Distribusi ini condong ke kiri (left-skewed).
* Penjelasan: Sebagian besar siswa memiliki persentase kehadiran yang tinggi, dengan puncak berada di antara 85% hingga 95%. Ada juga lonjakan di dekat 100%. Ini menunjukkan bahwa mayoritas siswa rajin hadir, dan hanya sedikit siswa yang memiliki persentase kehadiran rendah.

5. **sleep_hours (Jam Tidur):**
* Distribusi: Distribusi ini terlihat mendekati normal (approximately normal) atau sedikit condong ke kiri (slightly left-skewed).
* Penjelasan: Waktu tidur paling umum bagi siswa adalah sekitar 6 hingga 7 jam per malam. Distribusinya cukup simetris di sekitar puncak ini, meskipun ada sedikit kecenderungan siswa tidur lebih lama daripada lebih pendek dari rata-rata.

**6. exam_score (Nilai Ujian):**
* Distribusi: Distribusi ini juga tampak mendekati normal (approximately normal), mungkin sedikit condong ke kiri (slightly left-skewed).
* Penjelasan: Nilai ujian sebagian besar siswa terkonsentrasi di tengah, dengan puncak sekitar 65 hingga 75. Ada sebaran yang cukup merata di kedua sisi puncak, meskipun ada sedikit lebih banyak siswa dengan nilai di atas puncak dibandingkan di bawahnya. Ada juga kelompok kecil siswa yang mendapat nilai sangat tinggi (mendekati 100).
...
"""Insight:
Heatmap korelasi di atas sudah terlihat jelas bahwa fitur study_hours_per_day berkorelasi kuat terhadap fitur exam_score. itu artinya semakin lama/banyak seorang siswa menghabiskan waktunya untuk belajar dalam sehari maka semakin tinggi pula nilai ujian yang diperolehnya.
...
Output di atas adalah detail jumlah data dari setiap fitur kategorik yang divisualisasikan pada bar chart sebelumnya.
...
Missing value sudah berhasil diatasi menggunakan teknik imputasi dengan modus (data yang sering muncul), penggunaan modus sebagai metodek pengisian dikarenakan fitur tersebut bertipe kategori.
